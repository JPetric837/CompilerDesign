...
Generator::gen(If_then_stmt const* s)
{
  ...
  // create blocks for then and next
  ...
  // set the insertion point to 'next' branch
...
Generator::gen(If_else_stmt const* s)
{
  ...
  // create a blocks for then, else and next
  llvm::BasicBlock* _then = llvm::BasicBlock::Create(cxt, "then", fn);
...
{
  // create a blocks for condition, body and next
