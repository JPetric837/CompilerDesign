...
Lexer::Lexer() {
	...
	symtab.put("true", new Symbol("true", t_bool));
	symtab.put("false", new Symbol("false", t_bool));
...
Token * Lexer::true_tok() {
	if(peek(1) == 'r' && peek(2) == 'u' && peek(3) == 'e') {
		Token * tok = new Token(t_bool, symtab.get("true"));
		index += 4;
		return tok;
	}
	else
...
Token * Lexer::false_tok() {
	if(peek(1) == 'a' && peek(2) == 'l' && peek(3) == 's' && peek(4) == 'e') {
		Token * tok = new Token(t_bool, symtab.get("false"));
		index += 5;
		return tok;
	}
	else
